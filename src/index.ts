...
class LoopSDK {
  ...
  async connect() {
    ...
    const reused = await this.tryRestoreSession({ allowTicketFallback: true });
    if (reused) {
      return;
    }
  ...
  async autoConnect(): Promise<Provider | null> {
    ...
    const reused = await this.tryRestoreSession({ allowTicketFallback: false });
    return reused ? this.provider : null;
  }
  ...
  private createProviderHooks(): ProviderHooks {
    return {
      onRequestStart: async () => {
        // if the session is invalid, emit onLogout and let the integrator decide what to do
  ...
  private async tryRestoreSession({ allowTicketFallback }: { allowTicketFallback: boolean }): Promise<boolean> {
    ...
    try {
      const { ticketId, authToken, partyId, publicKey, email, connected } = JSON.parse(existingConnectionRaw);
      ...
      if (allowTicketFallback && ticketId) {
        this.ticketId = ticketId;
        const connectUrl = this.buildConnectUrl(ticketId);
        this.showQrCode(connectUrl);
        this.connection!.connectWebSocket(ticketId, this.handleWebSocketMessage.bind(this));
